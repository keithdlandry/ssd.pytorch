...
class LookaheadSSD(nn.Module):
    ...
    def __init__(self, phase, base, extras, head, num_classes, configs, network_name):
        super(LookaheadSSD, self).__init__()
        self.phase = phase
        ...
        self.el_sum_module_conv4 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.el_sum_module_conv_trans = nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2)
        self.el_sum_module_conv5 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.el_sum_module_norm = L2Norm(512, 10)
        self.el_sum_module_relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        # branch 1 of element wise sum module
        output_conv4_3 = self.el_sum_module_conv4(x)
        output_conv4_3 = self.L2Norm(output_conv4_3)

        # continue applying vgg up to conv5_3 ReLU
        for k in range(23, 30):
            x = self.vgg[k](x)

        # branch 2 of element wise sum module
        output_conv5_3 = self.el_sum_module_conv_trans(x)
        output_conv5_3 = self.el_sum_module_conv5(output_conv5_3)
        output_conv5_3 = self.el_sum_module_norm(output_conv5_3)

        # fuse feature maps from 4_3 and 5_3
        output_conv4_3 = self.el_sum_module_relu(output_conv4_3 + output_conv5_3)
        sources.append(output_conv4_3)
        del output_conv5_3
...
def build__lookahead_ssd(phase, configs, network_name='300', num_classes=21, square_boxes=False):
    ...
    return LookaheadSSD(phase, base_, extras_, head_, num_classes, configs, network_name)
